"""
FASE 5: ENTRENAMIENTO CAUSA (ISO 14224) - MODELO SINGLE-HEAD
============================================================
Entrena un clasificador de causa usando etiquetas normalizadas ISO
generadas por el LLM (archivo Consolidado_de_Resultados_Basados_en_IA_ISO.xlsx).
"""
...
CONFIG = {
    ...
    "val_split": 0.1,
    "test_split": 0.1,
    "data_path": "../data/Consolidado_de_Resultados_Basados_en_IA_ISO_v2.xlsx",
    "data_sheet": "Consolidado Base ISO v2",
    "output_model": "causa_iso_model.bin",
    "output_report": "training_report_causa_iso.md",
    "label_encoder": "label_encoder_causa_iso.json",
    "classification_report": "classification_report_causa_iso.txt",
}
...
class FocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        loss = -focal_weight.gather(1, targets.unsqueeze(1)).squeeze(1) * log_probs.gather(1, targets.unsqueeze(1)).squeeze(1)
        return loss.mean()


class TrainingVisualizer:
    def __init__(self, output_dir, prefix="causa", frame_every_steps=50, smooth_window=50):
...
def build_text(row):
    ...
    modo = str(row.get("modo_iso_label", ""))
    equipo = str(row.get("equipo", ""))
    denom = str(row.get("Denominación", ""))
    return f"{descripcion}. Problema: {problema}. Modo: {modo}. Equipo: {equipo}. Denominacion: {denom}."


def load_data():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(script_dir, CONFIG["data_path"])
    df = pd.read_excel(data_path, sheet_name=CONFIG["data_sheet"])

    df = df[df["causa_iso_label"].notna()].copy()

    # collapse rare classes
    counts = df["causa_iso_label"].value_counts()
    rare = counts[counts < CONFIG["min_class_count"]].index
    if len(rare) > 0:
        df.loc[df["causa_iso_label"].isin(rare), "causa_iso_label"] = "Other"

    if CONFIG["drop_other"]:
        df = df[df["causa_iso_label"] != "Other"].copy()

    print(f"Clases despues de filtros: {df['causa_iso_label'].nunique()} | Registros: {len(df)}")
    print(df["causa_iso_label"].value_counts().head(10).to_string())

    df["texto_completo"] = df.apply(build_text, axis=1)

    le = LabelEncoder()
    df["label_encoded"] = le.fit_transform(df["causa_iso_label"].astype(str))

    encoders_path = os.path.join(script_dir, CONFIG["label_encoder"])
    with open(encoders_path, "w", encoding="utf-8") as f:
        json.dump({"causa_iso_label": list(le.classes_)}, f, ensure_ascii=False, indent=2)
...
def create_dataloaders(df, tokenizer):
    ...
    train_ds = CausaDataset(texts[train_idx], labels[train_idx], tokenizer, CONFIG["max_length"])
    val_ds = CausaDataset(texts[val_idx], labels[val_idx], tokenizer, CONFIG["max_length"])
    test_ds = CausaDataset(texts[test_idx], labels[test_idx], tokenizer, CONFIG["max_length"])
    ...
    return train_loader, val_loader, test_loader, (labels[train_idx], labels[val_idx], labels[test_idx])
...
def train_epoch(model, loader, optimizer, scheduler, device, class_weights=None, epoch=None, visualizer=None):
    ...
    if CONFIG.get("use_focal_loss", False):
        criterion = FocalLoss(gamma=CONFIG.get("focal_gamma", 2.0), weight=class_weights)
    else:
...
def generate_report(metrics_history, final_metrics, le, training_time):
    ...
    report = f"""# REPORTE ENTRENAMIENTO CAUSA ISO
...
- Min class count: {CONFIG['min_class_count']}
- Drop Other: {CONFIG['drop_other']}
...
def train():
    ...
    train_loader, val_loader, test_loader, y_split = create_dataloaders(df, tokenizer)
    ...
    # Optional: save classification report to console
    print(classification_report(true, preds, target_names=le.classes_, zero_division=0))
    print(f"Modelo guardado en: {model_path}")
    print(f"Reporte guardado en: {report_path}")
    report_txt = os.path.join(script_dir, CONFIG["classification_report"])
    with open(report_txt, "w", encoding="utf-8") as f:
        f.write(classification_report(true, preds, target_names=le.classes_, zero_division=0))
    print(f"Classification report guardado en: {report_txt}")
    if visualizer:
